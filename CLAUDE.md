...
`frontend/script.js` → `POST /api/query` → `RAGSystem.query()` (`backend/rag_system.py`) → `AIGenerator.generate_response()` (`backend/ai_generator.py`), which calls Claude **with the `search_course_content` tool made available**. Claude decides whether to call the tool (system prompt: search only for course-specific questions, one search per query). If it does, `AIGenerator` executes the tool call through `ToolManager` → `CourseSearchTool.execute()` (`backend/search_tools.py`) → `VectorStore.search()` (`backend/vector_store.py`), feeds results back to Claude, and returns the final synthesized answer. Sources used in the answer are tracked on the tool instance (`CourseSearchTool.last_sources`) and returned alongside the answer to the frontend for citation display, then reset.
